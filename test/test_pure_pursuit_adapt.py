...
while True:
    ...
    tracking_error = math.sqrt((pos[0] - closest_wp[0]) ** 2 + (pos[1] - closest_wp[1]) ** 2)

    # Adaptive look-ahead distance
    base_look_ahead = 0.5 * current_speed  # 0.3 seconds look-ahead time
    look_ahead_dist = max(0.8, base_look_ahead * (1 + 0.5 * tracking_error))

    # Adaptive velocity
    adaptive_velocity = speed / (1 + 2.0 * tracking_error)

    # Compute steering and apply control
    steering_angle = compute_pure_pursuit_steering(pos, yaw, look_ahead_dist)
    apply_ackermann_control(steering_angle, adaptive_velocity)
